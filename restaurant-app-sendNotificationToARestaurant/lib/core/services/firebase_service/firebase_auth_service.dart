...
// import 'package:restaurant_app/core/managers/constants.dart';
...
class FirebaseAuthenticationService {
    ...
    try {
      ...
      if (googleSignInAccount == null) {
        ...
        return FirebaseAuthenticationResult.error(
          errorMessage: 'Process is canceled by the user',
        );
      }
  ...
  Future<FirebaseAuthenticationResult> signInWithFacebook() async {
    ...
    try {
      ...
      // Link the pending credentRial with the existing account
      if (_pendingCredential != null) {
        await result.user?.linkWithCredential(_pendingCredential!);
        _clearPendingData();
      }
      ...
      return FirebaseAuthenticationResult(
        uid: result.user?.uid,
        facebookAcessToken: loginResult.accessToken?.token,
        isNewUser: result.additionalUserInfo?.isNewUser,
        userToken: result.user?.refreshToken,
      );
    } on FirebaseAuthException catch (e) {
      ...
      log?.e(e);
      print('return FirebaseAuthenticationResult.error');
      return FirebaseAuthenticationResult.error(errorMessage: e.toString());
    }
  }
  ...
  Future<FirebaseAuthenticationResult> createAccountWithEmail(
    ...
    try {
      ...
      log?.d('Verification email sent: ${result.credential} ${result.user}');
...
class FirebaseAuthenticationResult {
  ...
  /// The authentication token associated with the user for facebook
  final String? facebookAcessToken;

  /// Verify if the user is new or not (for social Auth)
  ...
  FirebaseAuthenticationResult(
      {this.uid, this.userToken, this.isNewUser, this.facebookAcessToken})
      : errorMessage = null;
...
String getErrorMessageFromFirebaseException(FirebaseException exception) {
  switch (exception.code.toLowerCase()) {
    case 'email-already-in-use':
      return 'An account already exists for the email you\'re trying to use. Login instead or use another email.';
    case 'credential-already-in-use':
      return 'An account already exists for the credential you\'re trying to use. Login instead.';
    case 'invalid-email':
