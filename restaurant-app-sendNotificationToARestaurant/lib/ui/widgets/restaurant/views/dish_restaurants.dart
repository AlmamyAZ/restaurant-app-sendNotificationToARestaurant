...
class DishRestaurants extends ViewModelWidget<FoodViewModel> {
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate(
          (BuildContext context, int idx) {
            Restaurant restaurant = model.busy(model.restaurantsForDish)
                ? Restaurant()
                : model.restaurantsForDish![idx];
            return model.busy(model.restaurantsForDish)
          ...
          childCount: model.busy(model.restaurantsForDish)
              ? 5
              : model.restaurantsForDish?.length,
        ),
      ),
    );
  }
